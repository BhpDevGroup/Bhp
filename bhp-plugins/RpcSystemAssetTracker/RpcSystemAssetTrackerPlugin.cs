...
namespace Bhp.Plugins
{
    ...
    public class RpcSystemAssetTrackerPlugin : Plugin, IPersistencePlugin, IRpcPlugin
    {
        ...
        private DB _db;
        private DataCache<UserSystemAssetCoinOutputsKey, UserSystemAssetCoinOutputs> _userUnspentCoinsBhp;
        ...
        private void ResetBatch()
        {
            ...
            _userUnspentCoinsBhp = new DbCache<UserSystemAssetCoinOutputsKey, UserSystemAssetCoinOutputs>(_db, dbOptions,
                _writeBatch, SystemAssetUnspentCoinsPrefix);
            if (!_shouldTrackUnclaimed) return;
            _userSpentUnclaimedCoinsBhp = new DbCache<UserSystemAssetCoinOutputsKey, UserSystemAssetCoinOutputs>(_db, dbOptions,
        ...
        private bool ProcessBlock(Snapshot snapshot, Block block)
        {
            ...
            foreach (Transaction tx in block.Transactions)
            {
                ...
                {
                    UserSystemAssetCoinOutputs outputs = _userUnspentCoinsBhp.GetAndChange(
                        new UserSystemAssetCoinOutputsKey(output.AssetId, output.ScriptHash, tx.Hash),
                ...
                if (_shouldTrackUnclaimed && tx is ClaimTransaction claimTransaction)
                {
                    foreach (CoinReference input in claimTransaction.Claims)
                    {
                        ...
                        if (spentUnclaimedOutputs.AmountByTxIndex.Count == 0)
                            _userSpentUnclaimedCoinsBhp.Delete(claimedCoinKey);
        ...
        public void OnCommit(Snapshot snapshot)
        {
            ...
            _userUnspentCoinsBhp.Commit();
            if (_shouldTrackUnclaimed) _userSpentUnclaimedCoinsBhp.Commit();
        ...
        private JObject ProcessGetUnspents(JArray _params)
        {
            ...
            json["address"] = scriptHash.ToAddress();
            byte[] prefix = assetId.ToArray().Concat(scriptHash.ToArray()).ToArray();
            ...
            balance["asset_hash"] = assetId.ToString();
    ...
    public class RpcSystemAssetTrackerPlugin : Plugin, IPersistencePlugin, IRpcPlugin
    {
        ...
        private JObject ProcessGetUnspents(JArray _params)
        {
            ...
            string[] nativeAssetNames = {"BHPGas", "BHP"};
